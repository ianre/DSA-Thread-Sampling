...
class ImageProc:
    ...
    def SampleThread(self,polyLines, polyLineNames, needleEnd): 
        #thread_X = [0,0,0,0,0]
        #thread_Y = [0,0,0,0,0] 
        thread_X =[]
        thread_Y =[]
        ...
        curr_thread_X = [0,0,0,0,0]
        curr_thread_Y = [0,0,0,0,0]   
        for i in range(len(x_regular)): # draws each KeyPoint
            curr_thread_X[i] = x_regular[i]
            curr_thread_Y[i] = y_regular[i]
        return curr_thread_X, curr_thread_Y


    # This function needs to put the correct X and Y points in T_X[index] = [x1, x2, x3, x4, x5] 
    def generateThreadPoints_KT_S(self, labelSource, shortName,index, T_X, T_Y):
        J = JSONInterface(labelSource)
        polyNames , polygons = J.getPolygons(); # graspers only in KT, 
        kpNames, KeyPoint = J.getKeyPoints(); # None in KT,
        polyLineNames, polyLines = J.getPolyLines();
        ...
        if("Needle End" not in kpNames):
            #print("No needle for", labelSource)
            pass
        else: 
            ...
            if(len(polyLines) !=0):     
                Curr_T_X, Curr_T_Y = self.SampleThread(polyLines, polyLineNames, needleEnd)
        #T_X[index] = Curr_T_X
        #T_Y[index] = Curr_T_Y
        if(index<len(T_X[0])):
            for pointID in range(0,5): # for 5 points
                #print("Pid",pointID,"Idx",index)
                T_X[pointID][index] = Curr_T_X[pointID]
                T_Y[pointID][index] = Curr_T_Y[pointID]
        return T_X,T_Y
    ...
    def addAllVisData(self):
        ...
        for trial in all_kin_data:
            trial_s = trial.replace(".csv","")
            thisTrialDir = os.path.join(self.kinLabelDir,trial_s) # os.path.join(self.kinDir, "annotations_pre")
            trialKin = all_kin_data[trial]
            newFileName = os.path.join(self.kinOutput_Ext,trial_s) # os.path.join(self.kinDir, "output_ext")
            if not os.path.exists(thisTrialDir):
                print("No Annotations for ",thisTrialDir)
                countMissing = countMissing+1;
                continue;
                ...
                for g in range(1,6):
                    trialKin[0]+=",Thread_"+str(g)+"_X,Thread_"+str(g)+"_Y"
                ...
                T_X = [] # Thread Sample points. T_X[0] is the first point's sequence of X positions
                T_Y = []
                for i in range(0,5):
                    T_X.append(pre_inter_labels.copy())
                    T_Y.append(pre_inter_labels.copy())
                ...
                for root, dirs, files in os.walk(thisTrialDir):
                    for file in files:          
                        ...
                        T_X, T_Y = self.generateThreadPoints_KT_S(thisKinFileName,trial,f_n,T_X, T_Y)
                
                # Extension of first and last. Graspers, Needle, and Thread
                L_G_X = self.extendFirstLast(L_G_X)
                L_G_Y = self.extendFirstLast(L_G_Y)
                R_G_X = self.extendFirstLast(R_G_X)
                R_G_Y = self.extendFirstLast(R_G_Y)
                ...
                for pointID in range(0,5): # for 5 points
                        T_X[pointID] = self.extendFirstLast(T_X[pointID])
                        T_Y[pointID] = self.extendFirstLast(T_Y[pointID])

                # Interpolation Graspers, Needle, and Thread
                L_G_X = self.interpolate(L_G_X)
                L_G_Y = self.interpolate(L_G_Y)
                R_G_X = self.interpolate(R_G_X)
                R_G_Y = self.interpolate(R_G_Y)
                ...
                for pointID in range(0,5): # for 5 points
                        T_X[pointID] = self.interpolate(T_X[pointID])
                        T_Y[pointID] = self.interpolate(T_Y[pointID])
                ...
                if(self.task == "Needle_Passing"):                    
                    ...
                    for i in range(1,trialLength):
                        trialKin[i] = trialKin[i]+","+str(L_G_X[i])+","+str(L_G_Y[i])+","+str(R_G_X[i])+","+str(R_G_Y[i])+","+str(MID_N_X[i])+","+str(MID_N_Y[i])+","+str(R_4_X[i])+","+str(R_4_Y[i])+","+str(R_5_X[i])+","+str(R_5_Y[i])+","+str(R_6_X[i])+","+str(R_6_Y[i])+","+str(R_7_X[i])+","+str(R_7_Y[i]);
                        for pointID in range(0,5): # for 5 points
                            trialKin[i] += ","+str(T_X[pointID][i])+","+str(T_Y[pointID][i])
        ...
        for root, dirs, files in os.walk(self.kinSource):
            for file in files:
                ...
                for line in label_lines:
                    line = line.replace("\n","") 
                    l_angle, r_angle = self.getLRAnglesFromLine(line)
                    R_Angles.append(r_angle)
                    L_Angles.append(l_angle)
                    #if(r_angle == "1" and l_angle == "1"):
                    #    print("State",i,file)
                    Angles.append([l_angle,r_angle])
                    i=i+1
                label_len = i;                
                i = 0;
                for a_data in Angles:
                    i = i + 1;
                kin_data_left, kin_data_right = self.generateAngles(Angles,L_Angles,R_Angles);
                K_L = len(kin_data_left)
                K_R = len( kin_data_right)
                K_Lines = len( kin_lines)
                i = 0    
                kin_len = 0;                
                for line in kin_lines:
                    arr = line.split("\t")                    
                    if(arr[7] == "NaN"):
                        arr[7] = str(kin_data_left[i])
                    if(arr[15] == "NaN"):
                        arr[15] = str(kin_data_right[i])
                    if(arr[23] == "NaN"):
                        arr[23] = str(kin_data_left[i])
                    if(arr[31] == "NaN"):
                        arr[31] = str(kin_data_right[i])   
                    kin_lines[i] = "\t".join(arr)         
                    i=i+1
                self.save(out_file, kin_lines)  
                '''
...
class Iterator:
    ...
    def DrawLabel(self, imageSource, labelSource, target, debug=False):
        ...
        if(self.task == "Knot_Tying"):
            ...
            #for i in range(len(polyLines)):
            #    p = polyLines[i]
            #thread_X.sort(key=lambda x: x[0] + x[1])    
            #thread_X.sort(key=lambda x: math.sqrt(x[0]**2 + x[1]**2))    
            #thread_Y.sort(key=lambda x: x[0] + x[1])    
            #thread_Y.sort(key=lambda x: math.sqrt(x[0]**2 + x[1]**2))    
            ...
            for i in range(len(polyLines)):
                ...
                if("Top Left" in polyLineNames[i]):
                    for j in range(0,len(polyLines[i]),2):
                        Thread_TL_X.append(polyLines[i][j])
                        Thread_TL_Y.append(polyLines[i][j+1])
                elif("Top Right" in polyLineNames[i]):
                    for j in range(0,len(polyLines[i]),2):
                        Thread_TR_X.append(polyLines[i][j])
                        Thread_TR_Y.append(polyLines[i][j+1])
                elif("Bottom Left" in polyLineNames[i]):
                    for j in range(0,len(polyLines[i]),2):
                        Thread_BL_X.append(polyLines[i][j])
                        Thread_BL_Y.append(polyLines[i][j+1])
                elif("Bottom Right" in polyLineNames[i]):
                    for j in range(0,len(polyLines[i]),2):
                        Thread_BR_X.append(polyLines[i][j])
                        Thread_BR_Y.append(polyLines[i][j+1])
            ...
            T_X = [Thread_TL_X,Thread_TR_X,Thread_BL_X,Thread_BR_X]
            T_Y = [Thread_TL_Y,Thread_TR_Y,Thread_BL_Y,Thread_BR_Y]
            ...
            for i in range(len(T_Y)):
                ...
                if t_min!=0:
                    ...
                    #old_prev_length = prev_length[i];   
                    thread_delta[i] = abs(prev_length[i] - old_prev_length)
                    if(thread_delta[i] >200 and old_prev_length !=0):
                        continue;
                    ...
                    x_regular, y_regular = fx(alpha), fy(alpha)
                    for jj in range(0,len(T_X[i])):           
                        draw.line((T_X[i][kk],
                                T_Y[i][kk],
                                T_X[i][kk+1],
                                T_Y[i][kk+1]), fill=(255, 0, 0, 127), width=2) 
                        kk+=1
                        if(kk>=len(T_X[i])-1): break
            ...
            #polyLines.sort(key=lambda x: math.sqrt(x[0]**2 + x[1]**2))    
